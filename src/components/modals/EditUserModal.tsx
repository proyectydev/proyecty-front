...
export function EditUserModal({ isOpen, onClose, user }: EditUserModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    document_type: 'CC',
    document_number: '',
    user_type: 'user' as 'user' | 'investor' | 'borrower' | 'both' | 'admin',
    address: '',
    city: 'Cúcuta',
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        user_type: (user.user_type as 'user' | 'investor' | 'borrower' | 'both' | 'admin') || 'user',
        address: user.address || '',
        city: user.city || 'Cúcuta',
        department: user.department || 'Norte de Santander',
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
              ...
              <label htmlFor="is_active" className="flex-1">
                ...
                <p className="text-sm text-gray-500">Los usuarios inactivos no pueden acceder al sistema</p>
              </label>
              {formData.is_active ? (
